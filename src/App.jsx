...
/* ═══════════════════════════════════════════════════════════
   AI HOOK — Claude claude-sonnet-4-20250514
═══════════════════════════════════════════════════════════ */
function useAI() {
...
/* ═══════════════════════════════════════════════════════════
   AI SMART SUMMARY  (dashboard card)
═══════════════════════════════════════════════════════════ */
function AISmartSummary({ user }) {
  ...
  const PROMPTS = {
    ADMIN:    "Generate a concise 3-bullet executive business summary for a small industry. Cover: revenue health, workforce status, one risk. Use • bullets only. No headers.",
...
Modules: Employee management, Leave approvals, Payroll, Revenue analytics, AI warnings, Bulk email.
  ...
  return (
    <div style={{ position:"fixed", bottom:24, right:24, zIndex:1000, width:370, height:490, display:"flex", flexDirection:"column",
      ...
      <div style={{ padding:"13px 16px", borderBottom:"1px solid rgba(255,255,255,0.07)", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
        ...
        <button onClick={onClose} style={{ background:"rgba(255,80,80,0.1)", border:"1px solid rgba(255,80,80,0.28)", borderRadius:8, padding:"6px 10px", color:"#ff7a7a", fontSize:12, transition:"all 0.2s" }}>✕</button>
      </div>
      ...
      <div style={{ padding:"7px 12px", display:"flex", gap:6, overflowX:"auto", flexShrink:0 }}>
        {QUICK[user.role].map(q=>(
          <button key={q} onClick={()=>send(q)} style={{ whiteSpace:"nowrap", padding:"5px 11px", borderRadius:20, background:`${glow}10`, border:`1px solid ${glow}30`, color:glow, fontSize:11, fontWeight:600, transition:"all 0.15s" }}>{q}</button>
        ))}
      </div>
      ...
      <div style={{ padding:"10px 12px", borderTop:"1px solid rgba(255,255,255,0.06)", display:"flex", gap:7, flexShrink:0 }}>
        ...
        <button onClick={()=>send()} style={{ padding:"9px 13px", borderRadius:11, background:`${glow}18`, border:`1px solid ${glow}40`, color:glow, fontSize:15, transition:"all 0.15s" }}>➤</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ width:210, minHeight:"100vh", position:"relative", zIndex:10, flexShrink:0,
      ...
      <div style={{ padding:"12px 10px", borderTop:"1px solid rgba(255,255,255,0.05)" }}>
        <button onClick={onLogout} style={{ width:"100%", display:"flex", alignItems:"center", gap:9, padding:"10px 13px", borderRadius:11, border:"1px solid rgba(255,80,80,0.18)", background:"rgba(255,80,80,0.05)", color:"#ff8888", fontSize:12, fontWeight:500, transition:"all 0.18s" }}>🚪 Sign Out</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ height:62, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 26px", flexShrink:0, position:"relative", zIndex:10, background:"rgba(2,8,18,0.5)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(255,255,255,0.05)" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:11 }}>
        {/* AI button */}
        <button onClick={onAI} style={{ display:"flex", alignItems:"center", gap:7, padding:"7px 15px", borderRadius:11, background:`${glow}10`, border:`1px solid ${glow}30`, color:glow, fontSize:12, fontWeight:700, transition:"all 0.18s" }}>🤖 AI Assistant</button>
        {/* bell */}
...
function RevenueBar({ data }) {
  if (!data?.length) return <Card style={{ padding:22, display:"flex", alignItems:"center", justifyContent:"center", minHeight:200 }}><span style={{ color:"var(--mute)", fontSize:13 }}>No revenue data yet</span></Card>;
  const chart = [...data].reverse().slice(0,6).map(d=>({ name:MONTHS[d.month], Revenue:d.amount, Expense:d.expense }));
...
function AdminDash({ user }) {
  ...
  const [perf,     setPerf]     = useState([{ name:"Sales",pct:65 },{ name:"Marketing",pct:52 },{ name:"Ops",pct:48 },{ name:"Dev",pct:72 }]);
  const [loading,  setLoading]  = useState(true);

  useEffect(()=>{
    Promise.all([API.get("/api/admin/stats"), API.get("/api/admin/ai-warnings"), API.get("/api/admin/revenue")])
      .then(([s,w,r])=>{ setStats(s.data); setWarnings(w.data); setRevenue(r.data); })
  ...
  return (
    <div style={{ padding:22, display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:13 }}>
        <KPI icon="👥" label="Total Employees"  value={stats?.total_employees||0}            sub="In system"    color="var(--cyan)"   />
        <KPI icon="🧑‍💼" label="HR Users"          value={stats?.total_hr||0}                  sub="Active"       color="var(--orange)" />
        <KPI icon="💰" label="Monthly Payroll"  value={fmt(stats?.monthly_payroll)}          sub="This month"   color="var(--purple)" />
        <KPI icon="📈" label="Revenue"          value={fmt(stats?.revenue)}                  sub="This month"   color="var(--cyan)"   />
        <KPI icon="🏆" label="Net Profit"       value={fmt(stats?.profit)}                   sub="This month"   color="var(--orange)" />
        <KPI icon="📊" label="Salary / Revenue" value={`${stats?.salary_to_revenue_ratio||0}%`} sub="Ratio"    color="var(--purple)" />
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1.65fr 1fr", gap:13 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:13 }}>
          <Card style={{ padding:"18px 20px", flex:1 }}>
            ...
            <PerfBars data={perf.map(d=>({ name:d.name, value:d.pct }))} />
          </Card>
          <Card style={{ padding:"18px 20px" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
              ...
              {warnings.length===0 && <div style={{ color:"var(--mute)", fontSize:12, textAlign:"center", padding:12 }}>All clear ✓</div>}
            </div>
          </Card>
        </div>
      </div>
...
function HRMonitorPage() {
  ...
  const [loading, setLoading] = useState(true);
  useEffect(()=>{ API.get("/api/admin/activity-logs").then(r=>setLogs(r.data)).catch(console.error).finally(()=>setLoading(false)); },[]);
  const COLS = ["var(--cyan)","var(--purple)","var(--orange)","var(--yellow)","var(--red)"];
  return (
    <div style={{ padding:22, display:"flex", flexDirection:"column", gap:18 }}>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:13 }}>
        <KPI icon="✅" label="Activity Logs" value={logs.length} sub="Total"      color="var(--cyan)"   />
        <KPI icon="📅" label="Today"         value={logs.filter(l=>new Date(l.created_at).toDateString()===new Date().toDateString()).length} sub="Today" color="var(--orange)" />
        <KPI icon="👥" label="Active Users"  value="—" sub="All roles"            color="var(--purple)" />
        <KPI icon="📨" label="Emails Sent"   value="—" sub="This month"           color="var(--cyan)"   />
      </div>
      <Card style={{ padding:"20px 22px" }}>
        ...
        {loading ? <div style={{ display:"flex",justifyContent:"center",padding:20 }}><Spinner /></div> :
          <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
            ...
            {logs.length===0 && <div style={{ color:"var(--mute)", fontSize:13, textAlign:"center", padding:20 }}>No logs yet</div>}
          </div>
        }
      </Card>
    </div>
...
function RevenuePage() {
  ...
  const [form, setForm] = useState({ month:new Date().getMonth()+1, year:new Date().getFullYear(), amount:"", expense:"", notes:"" });

  const load = useCallback(()=>{
    Promise.all([API.get("/api/admin/revenue"), API.get("/api/admin/revenue-prediction")])
      .then(([r,p])=>{ setRevenue(r.data); setPred(p.data); }).catch(console.error).finally(()=>setLoading(false));
  },[]);
  useEffect(()=>{ load(); },[load]);
  const toast_ = (msg,type="success")=>{ setToast({msg,type}); setTimeout(()=>setToast(null),3000); };

  const save = async () => {
    try { await API.post("/api/admin/revenue",{ month:+form.month, year:+form.year, amount:+form.amount, expense:+form.expense, notes:form.notes }); toast_("Revenue saved!"); load(); }
    catch(e) { toast_(e.response?.data?.detail||"Error","error"); }
  };
  ...
  return (
    <div style={{ padding:22, display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:13 }}>
        <KPI icon="📈" label="Revenue"  value={lat?`₹${(lat.amount/100000).toFixed(1)}L`:"—"} sub="Latest" color="var(--cyan)"   />
        <KPI icon="📉" label="Expense"  value={lat?`₹${(lat.expense/100000).toFixed(1)}L`:"—"} sub="Latest" color="var(--orange)" />
        <KPI icon="🏆" label="Profit"   value={lat?`₹${(lat.profit/100000).toFixed(1)}L`:"—"}  sub="Latest" color="var(--purple)" />
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1.6fr 1fr", gap:13 }}>
        {loading ? <Card style={{ padding:40,display:"flex",justifyContent:"center" }}><Spinner /></Card> : <RevenueBar data={revenue} />}
        <div style={{ display:"flex", flexDirection:"column", gap:13 }}>
          {prediction && (
            <Card glow="var(--purple)" style={{ padding:"18px 20px" }}>
              ...
              <div style={{ padding:"14px", background:"rgba(167,139,250,0.07)", border:"1px solid rgba(167,139,250,0.22)", borderRadius:12 }}>
                ...
                <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:22, color:"var(--purple)" }}>₹{(prediction.predicted/100000).toFixed(1)}L</div>
                <div style={{ fontSize:11, color:"var(--mute)", marginTop:3 }}>{prediction.growth_pct}% growth · {prediction.confidence}% confidence</div>
              </div>
            </Card>
          )}
          <Card style={{ padding:"18px 20px" }}>
...
function HRDash({ user }) {
  const [stats, setStats]   = useState(null);
  const [loading, setLoading] = useState(true);
  useEffect(()=>{ API.get("/api/hr/stats").then(r=>setStats(r.data)).catch(console.error).finally(()=>setLoading(false)); },[]);
  const fmt = n => n>=100000 ? `₹${(n/100000).toFixed(1)}L` : `₹${(n||0).toLocaleString("en-IN")}`;
  if (loading) return <div style={{ padding:40,display:"flex",justifyContent:"center" }}><Spinner /></div>;
  return (
    <div style={{ padding:22, display:"flex", flexDirection:"column", gap:18 }}>
      <AISmartSummary user={user} />
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:13 }}>
        <KPI icon="👥" label="Employees"       value={stats?.total_employees||0}   sub="Active"       color="var(--cyan)"   />
        <KPI icon="📅" label="Pending Leaves"  value={stats?.pending_leaves||0}    sub="Review"       color="var(--orange)" />
        <KPI icon="⏳" label="Payroll Pending" value={stats?.pending_payroll||0}   sub="This month"   color="var(--purple)" />
        <KPI icon="💰" label="Payroll Total"   value={fmt(stats?.monthly_expense)} sub="This month"   color="var(--cyan)"   />
      </div>
    </div>
  );
}
...
function EmployeesPage() {
  ...
  const [toast,   setToast]   = useState(null);
  const [form, setForm] = useState({ name:"",email:"",password:"",department:"",position:"",base_salary:"",phone:"",address:"" });
  const COLS = ["var(--purple)","var(--orange)","var(--cyan)","var(--yellow)","var(--red)","#34d399"];

  const load = useCallback(()=>{ API.get("/api/hr/employees").then(r=>setEmps(r.data)).catch(console.error).finally(()=>setLoading(false)); },[]);
  useEffect(()=>{ load(); },[load]);
  const toast_ = (msg,type="success")=>{ setToast({msg,type}); setTimeout(()=>setToast(null),3000); };

  const add = async () => {
    try { await API.post("/api/hr/employees",{...form,base_salary:+form.base_salary}); toast_("Employee added!"); setShowAdd(false); setForm({name:"",email:"",password:"",department:"",position:"",base_salary:"",phone:"",address:""}); load(); }
    catch(e) { toast_(e.response?.data?.detail||"Error","error"); }
  };
  const del = async (id) => {
    if (!window.confirm("Deactivate?")) return;
    try { await API.delete(`/api/hr/employees/${id}`); toast_("Deactivated!"); load(); }
    catch { toast_("Error","error"); }
  };
      ...
      {showAdd && (
        <Card glow="var(--cyan)" style={{ padding:22, marginBottom:18, animation:"fadein 0.25s ease" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:11 }}>
            ...
            <Field label="Password"    value={form.password}    onChange={e=>setForm(p=>({...p,password:e.target.value}))}    type="password" />
            ...
            <Field label="Phone"       value={form.phone}       onChange={e=>setForm(p=>({...p,phone:e.target.value}))}       placeholder="+91 98765…" />
            <Field label="Address"     value={form.address}     onChange={e=>setForm(p=>({...p,address:e.target.value}))}     placeholder="City, State" />
          </div>
        ...
        <Card style={{ overflow:"hidden" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
            ...
            <tbody>
              ...
              {filtered.length===0 && (
                <tr><td colSpan={6} style={{ padding:30, textAlign:"center", color:"var(--mute)", fontSize:13 }}>No employees found</td></tr>
              )}
            </tbody>
          </table>
        </Card>
...
function LeavesPage() {
  ...
  const load = useCallback(()=>{ API.get(`/api/hr/leaves?status=${filter}`).then(r=>setLeaves(r.data)).catch(console.error).finally(()=>setLoading(false)); },[filter]);
  useEffect(()=>{ setLoading(true); load(); },[load]);
  const toast_ = (msg,type="success")=>{ setToast({msg,type}); setTimeout(()=>setToast(null),3000); };

  const review = async (id,status) => {
    try { await API.patch(`/api/hr/leaves/${id}/review`,{status}); toast_(`Leave ${status.toLowerCase()}!`); load(); }
    catch { toast_("Error","error"); }
  };
    ...
    <div style={{ padding:22 }}>
      ...
      {loading ? <div style={{ display:"flex",justifyContent:"center",padding:40 }}><Spinner /></div> :
        <div style={{ display:"flex", flexDirection:"column", gap:11 }}>
          {leaves.map(l=>(
            <Card key={l.id} style={{ padding:"15px 18px" }}>
              <div style={{ display:"flex", alignItems:"flex-start", gap:13 }}>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:14, fontWeight:700, color:"#fff" }}>Leave #{l.id}</div>
                  <div style={{ fontSize:11, color:"var(--mute)", marginTop:3 }}>{l.leave_type} · {l.days} days · {l.start_date} → {l.end_date}</div>
          ...
          {leaves.length===0 && <div style={{ color:"var(--mute)", fontSize:13, textAlign:"center", padding:40 }}>No {filter.toLowerCase()} leaves</div>}
        </div>
      }
    </div>
  );
...
function PayrollPage() {
  ...
  const { ask, aiLoading }      = useAI();
  const now = new Date();
  const [form, setForm] = useState({ employee_id:"", month:now.getMonth()+1, year:now.getFullYear(), base_salary:"", bonus:"0", deductions:"0" });
  const COLS = ["var(--purple)","var(--orange)","var(--cyan)","var(--yellow)"];

  const load = useCallback(()=>{
    Promise.all([API.get(`/api/hr/payroll?month=${now.getMonth()+1}&year=${now.getFullYear()}`), API.get("/api/hr/employees")])
      .then(([p,e])=>{ setPayrolls(p.data); setEmps(e.data); }).catch(console.error).finally(()=>setLoading(false));
  },[]);
  useEffect(()=>{ load(); },[load]);
  const toast_ = (msg,type="success")=>{ setToast({msg,type}); setTimeout(()=>setToast(null),3000); };

  const add = async () => {
    try { await API.post("/api/hr/payroll",{employee_id:+form.employee_id,month:+form.month,year:+form.year,base_salary:+form.base_salary,bonus:+form.bonus||0,deductions:+form.deductions||0}); toast_("Payroll created!"); setShowAdd(false); load(); }
    catch(e) { toast_(e.response?.data?.detail||"Error","error"); }
  };
  const markPaid = async (id) => {
    try { await API.post(`/api/hr/payroll/${id}/mark-paid`); toast_("Marked paid! Slip sent 📧"); load(); }
    catch { toast_("Error","error"); }
  };
  ...
  const aiProcess = async () => {
    ...
    const r = await ask(
      "You are a payroll AI. Write a professional 2-sentence payroll confirmation including amounts and date. Be concise.",
      `Process payroll for ${emps.length} employees. Total pool: ₹${total.toLocaleString("en-IN")}. Pending: ₹${pending.toLocaleString("en-IN")}. Date: ${now.toDateString()}.`
    );
    setAiMsg(r);
  };

  const net = (parseFloat(form.base_salary)||0) + (parseFloat(form.bonus)||0) - (parseFloat(form.deductions)||0);
    ...
    <div style={{ padding:22 }}>
      ...
      {showAdd && (
        <Card glow="var(--cyan)" style={{ padding:22, marginBottom:18, animation:"fadein 0.25s ease" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:11 }}>
            <Drop label="Employee" value={form.employee_id} onChange={e=>setForm(p=>({...p,employee_id:e.target.value}))} options={[{value:"",label:"Select employee"},...emps.map(e=>({value:e.id,label:e.user.name}))]} />
      ...
      {loading ? <div style={{ display:"flex",justifyContent:"center",padding:40 }}><Spinner /></div> :
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          {payrolls.length===0 && <div style={{ color:"var(--mute)", fontSize:13, textAlign:"center", padding:40 }}>No payroll entries yet</div>}
        </div>
      }
    </div>
  );
...
function CommsPage() {
  ...
  const send = async () => {
    ...
    try {
      ...
      if (dept!=="all") fd.append("department",dept);
      await API.post("/api/hr/send-email",fd);
  ...
  return (
    <div style={{ padding:22 }}>
      ...
      <Card style={{ padding:24, maxWidth:560 }}>
        ...
        <Btn color="var(--cyan)" onClick={send} disabled={sending} style={{ width:"100%", textAlign:"center" }}>
          {sending ? <Spinner size={13}/> : "Send Email 📨"}
        </Btn>
      </Card>
    </div>
  );
...
function EmpDash({ user }) {
  ...
  const [loading, setLoading] = useState(true);
  useEffect(()=>{ API.get("/api/employee/dashboard").then(r=>setData(r.data)).catch(console.error).finally(()=>setLoading(false)); },[]);
  if (loading) return <div style={{ padding:40,display:"flex",justifyContent:"center" }}><Spinner /></div>;
  return (
    <div style={{ padding:22, display:"flex", flexDirection:"column", gap:18 }}>
      <AISmartSummary user={user} />
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:13 }}>
        <KPI icon="🌿" label="Leave Balance" value={`${data?.leave_balance||0} days`} sub="Available"   color="var(--cyan)"   />
        <KPI icon="📅" label="Used Leave"    value={`${data?.used_leave||0} days`}    sub="This year"   color="var(--yellow)" />
        <KPI icon="⏳" label="Pending Leave" value={data?.pending_leave||0}           sub="Awaiting HR" color="var(--orange)" />
        <KPI icon="💰" label="Last Salary"   value={data?.last_salary?`₹${data.last_salary.toLocaleString("en-IN")}`:"—"} sub={data?.last_salary_month||"—"} color="var(--purple)" />
      </div>
    </div>
  );
}
...
/* ═══════════════════════════════════════════════════════════
   MY LEAVE (Employee) — AI confirmation
═══════════════════════════════════════════════════════════ */
function MyLeavePage({ user }) {
  ...
  const load = useCallback(()=>{ API.get("/api/employee/leaves").then(r=>setLeaves(r.data)).catch(console.error).finally(()=>setLoading(false)); },[]);
  useEffect(()=>{ load(); },[load]);
  const toast_ = (msg,type="success")=>{ setToast({msg,type}); setTimeout(()=>setToast(null),3000); };
  ...
  const apply = async () => {
    try {
      ...
      const conf = await ask(
        "You are an HR AI. Respond to a leave submission with a friendly 2-sentence confirmation message.",
        `Leave: ${form.leave_type}, From: ${form.start_date}, To: ${form.end_date}, Reason: ${form.reason||"Not provided"}.`
      );
      setAiConf(conf);
      toast_("Leave applied!"); setShow(false); load();
    } catch(e) { toast_(e.response?.data?.detail||"Error","error"); }
  };
    ...
    <div style={{ padding:22 }}>
      ...
      {show && (
        <Card glow={glow} style={{ padding:20, marginBottom:15, animation:"fadein 0.25s ease" }}>
          <Drop label="Leave Type" value={form.leave_type} onChange={e=>setForm(p=>({...p,leave_type:e.target.value}))} options={[{value:"SICK",label:"Sick Leave"},{value:"CASUAL",label:"Casual Leave"},{value:"ANNUAL",label:"Annual Leave"},{value:"OTHER",label:"Other"}]} />
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:11 }}>
...
function MyPayrollPage() {
  const [payrolls, setPayrolls] = useState([]);
  const [loading,  setLoading]  = useState(true);
  useEffect(()=>{ API.get("/api/employee/payroll").then(r=>setPayrolls(r.data)).catch(console.error).finally(()=>setLoading(false)); },[]);
  return (
    <div style={{ padding:22 }}>
      ...
      {loading ? <div style={{ display:"flex",justifyContent:"center",padding:40 }}><Spinner /></div> :
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          {payrolls.length===0 && <div style={{ color:"var(--mute)", fontSize:13, textAlign:"center", padding:40 }}>No payroll records yet</div>}
        </div>
      }
    </div>
  );
...
function ProfilePage({ user }) {
  ...
  useEffect(()=>{
    API.get("/api/employee/profile").then(r=>{ setProfile(r.data); setPhone(r.data.phone||""); setAddress(r.data.address||""); setEmergency(r.data.emergency_contact||""); }).catch(console.error).finally(()=>setLoading(false));
  },[]);
  const toast_ = (msg,type="success")=>{ setToast({msg,type}); setTimeout(()=>setToast(null),3000); };
  const save = async () => {
    try { await API.put("/api/employee/profile",{phone,address,emergency_contact:emergency}); toast_("Profile updated!"); }
    catch { toast_("Error","error"); }
  };
    ...
    <div style={{ padding:22 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"220px 1fr", gap:14 }}>
        <Card glow={glow} style={{ padding:"26px 18px", textAlign:"center" }}>
          ...
          <div style={{ fontSize:11, color:"var(--mute)", marginTop:3 }}>{profile?.department||"—"}</div>
          <div style={{ marginTop:9, display:"flex", flexDirection:"column", gap:5 }}>
            <Badge color={glow}>{LABEL[user.role]}</Badge>
            <Badge color="var(--purple)">{profile?.position||"—"}</Badge>
          </div>
        </Card>
        <Card style={{ padding:22 }}>
          ...
          {[["Email",user.email],["Department",profile?.department||"—"],["Position",profile?.position||"—"],["Base Salary",`₹${profile?.base_salary?.toLocaleString("en-IN")||"—"}`],["Join Date",profile?.join_date||"—"]].map(([l,v])=>(
          ...
          <Btn color={glow} onClick={save} style={{ marginTop:14 }}>Save Changes</Btn>
        </Card>
      </div>
    </div>
  );
...
function Login({ onLogin }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      onLogin({ id:d.user_id, name:d.name, email:d.email, role:d.role, avatar:d.name.split(" ").map(n=>n[0]).join("").slice(0,2).toUpperCase(), dept:"Company" });
    } catch(e) { setErr(e.response?.data?.detail||"Invalid credentials."); setLoading(false); }
  };
    ...
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:24, position:"relative" }}>
      ...
      <div style={{ position:"relative", zIndex:1, width:"100%", maxWidth:400, opacity:ready?1:0, transform:ready?"translateY(0)":"translateY(28px)", transition:"all 0.7s cubic-bezier(0.16,1,0.3,1)" }}>
        ...
        <Card glow="var(--cyan)" style={{ padding:"30px 28px" }}>
          ...
          <form onSubmit={submit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div>
              ...
              <div style={{ position:"relative" }}>
                ...
                <input type="email" placeholder="you@company.com" value={email} onChange={e=>{setEmail(e.target.value);setErr("");}} required
                  style={{ width:"100%", padding:"11px 13px 11px 36px", borderRadius:11, fontSize:13 }} />
              </div>
            </div>
            <div>
              ...
              <div style={{ position:"relative" }}>
                <span style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", opacity:0.35, fontSize:14 }}>🔒</span>
                <input type={show?"text":"password"} placeholder="••••••••" value={pass} onChange={e=>{setPass(e.target.value);setErr("");}} required
                  style={{ width:"100%", padding:"11px 38px 11px 36px", borderRadius:11, fontSize:13 }} />
          ...
          {/* role pills */}
          <div style={{ marginTop:20, paddingTop:16, borderTop:"1px solid rgba(255,255,255,0.06)", display:"flex", justifyContent:"center", gap:8 }}>
            {[["var(--orange)","ADMIN"],["var(--cyan)","HR"],["var(--purple)","EMP"]].map(([c,r])=>(
              <div key={r} style={{ display:"flex", alignItems:"center", gap:5, padding:"4px 10px", borderRadius:20, background:`${c}0f`, border:`1px solid ${c}28` }}>
                <div style={{ width:5,height:5,borderRadius:"50%",background:c,boxShadow:`0 0 6px ${c}` }} />
                <span style={{ fontSize:9, color:c, letterSpacing:"1px", fontWeight:700 }}>{r}</span>
              </div>
            ))}
          </div>
        </Card>
...
export default function App() {
  ...
  useEffect(()=>{
    const t = localStorage.getItem("access_token");
    if (t) API.get("/api/auth/me").then(r=>setUser({id:r.data.id,name:r.data.name,email:r.data.email,role:r.data.role,avatar:r.data.name.split(" ").map(n=>n[0]).join("").slice(0,2).toUpperCase(),dept:"Company"})).catch(()=>localStorage.clear());
  },[]);

  const logout = () => { localStorage.clear(); setUser(null); setPage("dashboard"); setAiOn(false); };
